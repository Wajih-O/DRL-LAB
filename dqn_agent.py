...
class DQNAgent(ExperienceReplayAgent):
    """Interacts with and learns from the environment."""

    def __init__(self, network_factory: NNFactory, replay_buffer_factory: BaseReplayBufferFactory,
        ...
        """
        super(DQNAgent, self).__init__(replay_buffer_factory=replay_buffer_factory, seed=seed,
                                       batch_size=batch_size,
                                       step_to_update=step_to_update, buffer_size=buffer_size,
                                       gamma=gamma, episodes_window_size=episodes_window_size,
                                       name=name)
    ...
    def explore(self, env, extract_state, n_episodes=1800, max_t=1000, brain_name=None,
        ...
        :param n_episodes: the max number of exploration episodes (env. expected to be solve before)
        :param max_t: maximum steps/actions within an episode (if termination is not reached)
    ...
    def save(self):
        # TODO save the full agent to be reloaded (implement a save/load mechanism)
        """ save local Q network_local as state_dict"""
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        self.memory.add(state, action, reward, next_state,
                        done, self.target_local_abs_error(state, action))
    ...
    def learn(self, experiences: List[Experience]):
        ...
        """
        states, actions, rewards, next_states, dones = to_tensors(experiences, DEVICE)
